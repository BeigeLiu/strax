...
def baseline(records, baseline_samples=40,flip=True,
    ...
    for d_i, d in enumerate(records):
        ...
        #print('record_i',d['record_i'])
        #print('channel number',d['channel'])
        #print('seen first',seen_first)
...
def integrate(records):
    ...
    for i, r in enumerate(records):
        #print('r',r['data'].sum() * 2**r['amplitude_bit_shift'])
        records[i]['area'] = (
...
def record_links(records):
    ...
    for i, r in enumerate(records):
        ...
        # print('========')
        # print('r time:',r['time'])
        # print('expected_next:',expected_next_start[ch])
        # print('==========')
...
def _find_hits(records, min_amplitude, min_height_over_noise,
    ...
    for record_i, r in enumerate(records):
        ...
        # print('\n')
        # print('=======================================')
        # print('record i',r['record_i'])
        # print('channel:',r['channel'])
        # print('time start-end',str(r['time'])+'-'+str(strax.endtime(r)))
        for i in range(n_samples):
            ...
            if in_interval:
                ...
                if not in_interval:
                    ...
                    res['record_i'] = record_i#*n_channels+r['channel']
                    ...
                    area = height = 0
                    # if not np.sum(r['data']) == 0:
                    #     print('====>hit_i:',str(res['record_i']))
                    #     print('=========>channel:',str(res['channel']))
                    #     print('=========>time start-end',str(res['time'])+'-'+str(strax.endtime(res)))
                    #     print('=========>hit left-right',str(res['left'])+'-'+str(res['right']))
                    #     print('=========>area',res['area'])
                    #     print('========record:',r['data'])
        ...
        #     if len(r_i) !=0:
        #         for k in range(8):
        #             ax.plot(r_i['data'],drawstyle = 'steps-mid',label = 'ch_'+str(k))
        #             ax.vlines(hit['left'],ymax=np.max(r_i['data']),
        #                     ymin=np.min(r_i['data']),
        #                     linestyles='dashed', colors='red')
        #             ax.vlines(hit['right'],
        #                     ymax=np.max(r_i['data']),
        #                     ymin=np.min(r_i['data']),
        #                     linestyles='dashed', colors='red')
        #             ax.legend()
        #             ax.grid()
        #         plt.savefig(r'C:\Users\lbg\Desktop\hit_'+str(record_i)+'.pdf')
        #         plt.close(fig)
