...
def find_peaks(hits, adc_to_pe,
    ...
    for hit_i, hit in enumerate(hits):
        ...
        # if peak_i == 0:
        #     # if hit['channel'] == 1:
        #         print('hit number:',hit_i)
        #         print('PMT number:',hit['channel'])
        #         print('=====>start-end:',str(hit['time'])+'-'+str(strax.endtime(hit)))
        #         print('=====>left-right:',str(hit['left'])+'-'+str(hit['right']))
        #         print('=====>height:',hit['height'])
        #         print('=====>PE',hit['area']* adc_to_pe[hit['channel']])
        #         print('\n')
        ...
        if is_last_hit or next_hit_is_far or peak_too_long:
            ...
            #     print('\n')
            #     print('=================')
            #     print('peak number:',peak_i)
            #     print('n_hits',p['n_hits'])
            #     print('peak_start:',p['time'])
            #     print('peak end:',peak_endtime)
            #     print('=================')
            #     print('\n')
...
def sum_waveform(peaks, hits, records, record_links, adc_to_pe, n_top_channels=0,
    ...
    for peak_i in select_peaks_indices:
        ...
        # print('hit',hits[left_h_i])
        # print('hit before',hits[left_h_i-1])
        # print('hit after ',hits[left_h_i+1])
        # print('p',p['time'],strax.endtime(p))
        for right_h_i in range(left_h_i, len(hits)):
            ...
            if shift <= -n_samples_peak:
                # r = records[record_i]
                # print('========================================================')
                # print('record begin-end',str(r['time'])+'-'+str(strax.endtime(r)))
                # print('hit begin-end',str(h['time'])+'-'+str(strax.endtime(h)))
                # print('peak begin-end',str(p['time'])+'-'+str(strax.endtime(p)))
            ...
            (h_start, h_end), (p_start, p_end) = strax.overlap_indices(
                (h['time']-p['time']) // dt, n_samples_hit,
                0, n_samples_peak)
            
            # print('=================================================')
            # print('peak begin - peak end, length',p['time'],strax.endtime(p),n_samples_peak)
            # print('hit  begin - hit end,length ',h['time'],strax.endtime(h),n_samples_hit)
            # print('dt',dt)
            # print('s ',(h['time']-p['time'])//dt)
            # print('peak overlap samples',p_start,p_end)
            # print('hit overlap samples',h_start,h_end)
...
def _build_hit_waveform(hit, record, hit_waveform):
    ...
    (h_start_record, h_end_record), (r_start, r_end) = strax.overlap_indices(
        (hit['time']-record['time']) // hit['dt'], hit['length'],
        0, record['length'])
    
    # print('hit time',hit['time'])
    ...
    # print(record_data)
    # print(not len(record_data))
    multiplier = 2**record['amplitude_bit_shift']
    bl_fpart = record['baseline'] % 1      ## baseline float part
    # print(bl_fpart)
    ...
    # Build hit waveform:
    # print(h_start_record,h_end_record)
    # hit_waveform[h_start_record:h_end_record] = (multiplier * record_data + bl_fpart)
    # if record['channel'] == 2:
    #     print('\n')
    #     print('================================')
    #     print('record_i & channel',record['record_i'],record['channel'])
    #     print('record',record['data'])
    #     print('h start-end:',str(h_start_record)+'-'+str(h_end_record))
    #     print('r start-end:',str(r_start)+'-'+str(r_end))
    #     print('record_data',record_data)
    #     print('hit waveform',hit_waveform)
