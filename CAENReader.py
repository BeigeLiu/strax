...
class DataFile:
    ...
    def _splittrigger(self,triggers):
        ...
        for trigger in triggers:
            split_into       = int(trigger['pulse_length'][0]//self.recordLen + 1)
    ...
    def _getNextTrigger(self):
        ...
        for ind, k in enumerate(whichChan):
            if k == 1: # whether the channel have data
                ...
                # convert from ticks to us since the beginning of the file
                trtime      = trtime * 8e-3
                this_trigger                 = np.zeros(1,dtype=raw_record_dtype(samples_per_record=20*self.recordLen))
                ...
                this_trigger['data'][0,:int(2*(chsize-3))] = np.expand_dims(fromfile(self.file, dtype=dt, count=int(2*(chsize-3))),
                                                                            axis = 0)
                if self.calcBSL:
                   this_trigger['baseline']  = np.mean(this_trigger['data'][0,:self.num_of_pre_samples])
                this_trigger['data'][0,int(2*(chsize-3)):] = this_trigger['baseline']
    ...
    def check_waveform(self,i,channel=None):
        record_to_plot = self.triggers[self.triggers['record_i']==i]
        if channel != None:
            record_to_plot = record_to_plot[record_to_plot['channel']==channel]
        import matplotlib.pyplot as plt
        fig = plt.figure()
        ax  = fig.add_subplot(111)
        for record_i in record_to_plot:
            time = np.linspace(record_i['time'],strax.endtime(record_i),len(record_i['data']))
            ax.plot(time,-(record_i['data'][:]-record_i['baseline'])/2**13*1e3,label ='PMT '+str(record_i['channel'])+'\n'\
                                                            'record '+str(record_i['record_i'])+'\n',
                                                    drawstyle = 'steps-mid')
        ax.hlines(50.0/2**13*1e3,np.min(time),np.max(time),linestyles='--',colors='red',label = 'hit threshold')
            #ax.hlines(10.0/2**13*1e3,np.min(time),np.max(time),linestyles='--',colors='blue',label = 'hit threshold')
        ax.grid()
        ax.set_xlabel('time(ns)')
        ax.legend(frameon = False)
        ax.set_ylabel('voltages(mV)')
        plt.savefig('/home/user/data_process/figures/raw_record_'+str(i)+'.pdf')
        return
    
# data = DataFile(fileName='/data/run0/run00003_raw_b0_seg0.bin')
# for i in range(10,20):
#     data.check_waveform(i)