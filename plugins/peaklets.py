...
class Peaklets(Plugin):
    ...
    def compute(self, records, start, end):
        ...
        #     z     = np.random.randint(1,np.max(r['record_i']))
        #     r_i   = r[r['record_i']==z]
        #     r_ch  = r_i[r_i['channel'] == channel_consider]
        #     hit   = hits[hits['record_i']==z*8+(channel_consider)] 
        #     hit_k = hit[hit['channel']==channel_consider]
        #     if not len(hit_k) == 0:
        #         i += 1
        #         print('\n')
        #         print('======================================================')
        #         print('record_i',z)
        #         print('data',r_ch['data'][0])
        #         print('channel',r_ch['channel'])
        #         print('hit channel',hit_k['channel'])
        #         print('hit height',hit_k['height'])
        #         fig = plt.figure(dpi=400)
        #         ax = fig.add_subplot(111)
        #         k = 0
        #         x = np.linspace(0,r_ch['data'].shape[1],r_ch['data'].shape[1])
        #         ax.plot(x,r_ch['data'][0,:],'r--',linewidth = 1,drawstyle = 'steps-mid',label = 'total event')
        #         for h in hit_k:
        #             # print(k)
        #             # print('hit wave',r_ch['data'][0,h['left']:h['right']])
        #             if k == 0:
        #                 #ax.plot(x[:h['left']],r_ch['data'][0,:h['left']],drawstyle = 'steps-mid',c = 'blue',label = 'baseline')
        #                 ax.plot(x[h['left']:h['right']],r_ch['data'][0,h['left']:h['right']],drawstyle = 'steps-mid',label = 'hit_'+str(k+1))
        #             elif k == len(hit_k)-1:
        #                 ax.plot(x[h['left']:h['right']],r_ch['data'][0,h['left']:h['right']],drawstyle = 'steps-mid',label = 'hit_'+str(k+1))
        #                 #ax.plot(x[h['right']:],r_ch['data'][0,h['right']:],drawstyle = 'steps-mid',c = 'blue')
        #             else:
        #                 ax.plot(x[h['left']:h['right']],r_ch['data'][0,h['left']:h['right']],drawstyle = 'steps-mid',label = 'hit_'+str(k+1))
        #             k += 1
        #             # print('left-right:',str(h['left'])+'-'+str(h['right']))
        #             ax.vlines(h['left'],
        #                     ymax=0,
        #                     ymin=1000,
        #                     linestyles='dashed', colors='blue',linewidth = 0.1)
        #             ax.vlines(h['right'],
        #                     ymax=0,
        #                     ymin=1000,
        #                     linestyles='dashed', colors='blue',linewidth = 0.1)
        ...
        #         ax.set_title('Record '+str(z)+' and the hits found in it',fontsize = 15)
        #         savename = r'C:\Users\lbg\Desktop\record_'+str(i)+'.png'
        #         print(savename)
        ...
        peaklets = strax.sort_by_time(peaklets)
        # print('============')
        # print(peaklets['time'])
        # print(records['time'])
        # print(peaklets['time'])
        ...
        # print(n_top_pmts_if_digitize_top)
